...
const AllUsers = () => {
  ...
  const handleAdmin = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://bistro-boss-restaurant-server-l2ff0sdju-diptapal.vercel.app/users/admin/${id}`, {
          method: "PATCH",
  ...
  const handleDelete = (user) => {
    if (user.role === "admin") {
      ...
      //     deleteUser(user)
      //       .then(() => {
      //         fetch(`https://bistro-boss-restaurant-server-l2ff0sdju-diptapal.vercel.app/users/${user._id}`, {
      //           method: "DELETE",
      //         })
      //           .then((res) => res.json())
      //           .then((data) => {
